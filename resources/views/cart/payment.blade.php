...
<head>
    ...
    <title>Pembayaran Snacks - Sansflix Cinema</title>
        ...
        <div class="w-full md:w-1/3 bg-gray-900 p-8 border-r border-gray-700 relative overflow-hidden">
            ...
            <div class="space-y-4 mb-6 max-h-60 overflow-y-auto pr-2">
                ...
                <div class="flex items-center gap-3">
                    <img src="{{ $item->foodBeverage->image_url }}" class="w-12 h-12 object-cover rounded-lg border border-gray-700">
                    <div class="flex-grow">
                        <h2 class="text-white font-bold text-sm leading-tight">{{ $item->foodBeverage->name }}</h2>
                        <p class="text-gray-400 text-[10px] uppercase tracking-tighter">{{ $item->quantity }} x Rp {{ number_format($item->price, 0, ',', '.') }}</p>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            <div class="space-y-3 text-sm text-gray-300 border-t border-gray-700 pt-4">
                <div class="flex justify-between">
                    ...
                    <span class="font-mono text-white tracking-wide">#FNB-{{ $order->id }}</span>
                </div>
                <div class="flex justify-between">
                    <span>Waktu Pesan</span>
                    <span class="text-white">{{ $order->created_at->format('d M, H:i') }}</span>
                </div>
            </div>

            <div class="mt-8 pt-4 border-t border-dashed border-gray-600">
                <p class="text-gray-400 text-sm mb-1">Total Tagihan</p>
                <h1 class="text-3xl font-extrabold text-white">Rp {{ number_format($order->total_price, 0, ',', '.') }}</h1>
            </div>
        </div>
        ...
        <div class="w-full md:w-2/3 p-8 relative bg-gray-50 text-gray-800">
            ...
            @if($order->status == 'pending')
                <div class="flex justify-between items-center mb-6">
                    ...
                    <span class="text-xs bg-red-100 text-red-600 px-2 py-1 rounded border border-red-200 animate-pulse font-semibold">Bayar Sekarang</span>
                </div>

                <form action="{{ route('cart.pay_proses', $order->id) }}" method="POST" id="payment-form">
                    @csrf
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
                        <label class="cursor-pointer relative">
                            ...
                            <div class="payment-card h-24 rounded-xl border border-gray-200 bg-white flex flex-col items-center justify-center transition-all group">
                                ...
                                <span class="text-xs text-gray-400 group-hover:text-black uppercase font-bold text-[9px]">Scan QR</span>
                        ...
                        <label class="cursor-pointer relative">
                            ...
                            <div class="payment-card h-24 rounded-xl border border-gray-200 bg-white flex flex-col items-center justify-center transition-all group">
                                ...
                                <span class="text-xs text-gray-400 group-hover:text-black uppercase font-bold text-[9px]">Instant Pay</span>
                                <div class="check-icon absolute top-2 right-2 text-blue-500 opacity-0 transition-opacity">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>
                                </div>
                            </div>
                        ...
                        <label class="cursor-pointer relative">
                            ...
                            <div class="payment-card h-24 rounded-xl border border-gray-200 bg-white flex flex-col items-center justify-center transition-all group">
                                ...
                                <span class="text-xs text-gray-400 group-hover:text-black uppercase font-bold text-[9px]">E-Wallet</span>
                                <div class="check-icon absolute top-2 right-2 text-blue-500 opacity-0 transition-opacity">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>
                                </div>
                            </div>
                        ...
                        <label class="cursor-pointer relative">
                            ...
                            <div class="payment-card h-24 rounded-xl border border-gray-200 bg-white flex flex-col items-center justify-center transition-all group">
                                ...
                                <span class="text-xs text-gray-400 group-hover:text-black uppercase font-bold text-[9px]">VA Transfer</span>
                                <div class="check-icon absolute top-2 right-2 text-blue-500 opacity-0 transition-opacity">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>
                                </div>
                            </div>
                        ...
                        <label class="cursor-pointer relative">
                            ...
                            <div class="payment-card h-24 rounded-xl border border-gray-200 bg-white flex flex-col items-center justify-center transition-all group">
                                ...
                                <span class="text-xs text-gray-400 group-hover:text-black uppercase font-bold text-[9px]">Virtual Account</span>
                                <div class="check-icon absolute top-2 right-2 text-blue-500 opacity-0 transition-opacity">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>
                                </div>
                            </div>
                    ...
                    <div id="payment-details" class="bg-white border border-gray-200 rounded-xl p-6 mb-6 hidden shadow-inner transition-all duration-300">
                        <div id="detail-qris" class="hidden detail-section text-center">
                            ...
                            <div class="bg-white p-3 inline-block rounded-lg mb-2 border shadow-sm">
                                <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=FNB-{{ $order->id }}" class="w-32 h-32">
                            </div>
                            <p class="text-[10px] text-gray-400 mt-2 font-mono">NMID: ID102030405060</p>
                        </div>
                        ...
                        <div id="detail-va" class="hidden detail-section">
                            ...
                            <div class="flex items-center justify-between bg-gray-100 p-3 rounded-lg border border-gray-300 mb-3">
                                ...
                                <button type="button" class="text-[10px] bg-white border px-3 py-1 rounded-md text-blue-500 hover:text-blue-700 uppercase font-bold shadow-sm">Salin</button>
                            </div>
                            <p class="text-[10px] text-gray-400 italic italic text-center">Gunakan menu Transfer Virtual Account pada Mobile Banking Anda.</p>
                        </div>

                        <div id="detail-ewallet" class="hidden detail-section text-center py-4">
                            <p class="text-sm text-gray-600 mb-2 font-bold">Konfirmasi Cepat Melalui HP</p>
                            <p class="text-xs text-gray-400 mb-4">Pastikan notifikasi aplikasi muncul di ponsel Anda.</p>
            ...
            @elseif($order->status == 'paid')
                <div class="h-full flex flex-col items-center justify-center py-10 text-center">
                    ...
                    <h2 class="text-3xl font-extrabold text-gray-800 mb-2">Yeay! Pembayaran Berhasil</h2>
                    <p class="text-gray-500 mb-8">Camilan Anda sedang disiapkan. Silakan cek menu Riwayat.</p>
                    ...
                    <div class="w-full bg-white border border-gray-200 rounded-xl p-4 mb-8 flex justify-between items-center shadow-sm">
                        ...
                        <span class="font-bold text-blue-600 uppercase tracking-wider text-sm">{{ $order->payment_method ?? 'CASHLESS' }}</span>
                    </div>

                    <div class="w-full">
                        <a href="{{ route('history') }}" class="block w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white font-bold py-4 rounded-xl transition shadow-lg flex items-center justify-center gap-2">
                             Lihat Pesanan Saya
                        </a>
                    </div>
                </div>
