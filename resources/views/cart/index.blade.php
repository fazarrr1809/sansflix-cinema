    ...
    <div class="container mx-auto px-4">
                ...
                <div class="lg:col-span-2">
                    <div class="bg-gray-900 rounded-2xl overflow-hidden border border-gray-800" id="cart-items-container">
                        ...
                        @foreach(session('cart') as $id => $details)
                            ...
                            <div class="flex flex-col md:flex-row items-center gap-4 p-6 border-b border-gray-800 last:border-0" id="item-row-{{ $id }}">
                                <img src="{{ $details['image'] }}" class="w-24 h-24 object-cover rounded-lg shadow-lg">
                                
                                <div class="flex-grow text-center md:text-left">
                                    <h3 class="font-bold text-lg">{{ $details['name'] }}</h3>
                                    <p class="text-yellow-500 font-semibold">Rp {{ number_format($details['price'], 0, ',', '.') }}</p>
                                </div>

                                {{-- FITUR TAMBAH KURANG JUMLAH --}}
                                <div class="flex items-center gap-3 bg-gray-950 p-2 rounded-xl border border-gray-800">
                                    <button onclick="updateCartQty({{ $id }}, -1)" 
                                    ...
                                    <span class="w-8 text-center font-bold text-lg" id="qty-{{ $id }}">
                                        {{ $details['quantity'] }}
                                    </span>
                                ...
                                <div class="text-right min-w-[120px]">
                                    {{-- TAMBAHKAN ID PADA SUBTOTAL ITEM --}}
                                    <p class="font-bold text-red-600 text-lg" id="subtotal-{{ $id }}">
                ...
                <div class="lg:col-span-1">
                    <div class="bg-gray-900 p-6 rounded-2xl border border-gray-800 sticky top-24 shadow-2xl">
                        ...
                        <div class="flex justify-between mb-4">
                            ...
                            <span class="font-bold text-white text-lg" id="summary-subtotal">Rp {{ number_format($total, 0, ',', '.') }}</span>
                        </div>
                        ...
                        <div class="border-t border-gray-800 pt-4 flex justify-between mb-8">
                            ...
                            <span class="font-black text-2xl text-yellow-500" id="summary-total">Rp {{ number_format($total, 0, ',', '.') }}</span>
                        </div>
                        ...
                        <a href="{{ route('fnb.index') }}" class="block text-center mt-4 text-sm text-gray-500 hover:text-white transition">
                            Tambah menu lainnya
                        </a>
                    </div>
                </div>
        ...
        @else
            <div class="text-center py-24 bg-gray-900 rounded-3xl border border-gray-800 shadow-xl">
                ...
                <a href="{{ route('fnb.index') }}" class="inline-block mt-6 bg-red-600 text-white px-8 py-3 rounded-full font-bold hover:bg-red-700 transition transform hover:scale-110 shadow-lg shadow-red-600/20">
                    Ayo Cari Snack!
                </a>
            </div>
        @endif
